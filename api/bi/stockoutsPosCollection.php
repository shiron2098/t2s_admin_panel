...
class stockoutsPosCollection  extends MYSQL_t2s_bi_Collection
{
    ...
    public function Week($date, $int, $offset, $count,$sort,$routekey,$groupkey)
    {
        if (!empty($date) && isset($date)) {
            ...
            $data = $this->select_array_stockouts_collection($timemysql,$offset,$count,$sort,$routekey,$groupkey);
            $dataCount= $this->select_count_stockouts($timemysql,$routekey,$groupkey);
            if ($data !== null) {
                $output = array(
                    'date' => $date,
